 ...
 *
 * @category       Models
 * @package        AuResto\Models\Updates_model.php
...
class Updates_model extends TI_Model {
	...
	protected $endpoint = 'https://api.auresto.com/v1';
	...
	public function update($update_type, $update_name = '', $update_version = '') {
		...
		if ($update_type === 'core') {
			...
			$filename = 'AuResto-' . $update_version;

			flush_output(sprintf($this->lang->line('progress_update'), 'AuResto'));
			flush_output($this->lang->line('progress_download_update'));
		} else {
			$extension = $this->Extensions_model->getExtension($update_name);

			$filename = 'AuResto-' . $update_type . '-' . $update_name;
	...
	public function installCoreUpdate($update_version, $update_file) {
		...
		if ( ! is_dir($temp_path)) {
			...
			$remove_files = array(
				'/tests',
				'/system/auresto/logs',
				'/system/auresto/session',
				'/system/auresto/config/database.php',
			);
	...
	public function flushUpdatedFiles() {
		...
		$html .= '<p>'.sprintf($this->lang->line('progress_update_success'), 'AuResto').'</p>';
		flush_output($html, FALSE);
	}
}

/* End of file Updates_model.php */
/* Location: ./system/auresto/models/Updates_model.php */